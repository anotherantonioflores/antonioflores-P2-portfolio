...
set.seed(24)
traindown = downSample(x=train_x, 
                       y=train_y)
...
# MARS model
set.seed(21)
marsmodel = train(Class~., data=traindown, 
                  method = "earth",
...
file_path <- here("results", "figures","DSimportantM1.png")
...
knnModel = train(Class~., data=traindown, 
                 method = "knn",
...
file_path <- here("results", "figures","DSimportantKNN1.png")
png(filename = file_path, width = 800, height = 600)
plot(KNNImp)
dev.off()
...
logregmodel = glm(Class~., data=traindown, family = "binomial")
...
ggsave(here("results", "figures","DSimportantLog1.png"))
...
rfmodel = randomForest(Class~., data = traindown,
                importance = TRUE,
                preProcess = "pca",
                trControl = ctrl)
...
ggsave(here("results", "figures","DSimportantRF1.png"))
...
table_file3 = here("results", "tables", "finaltable3.rds")
saveRDS(finalresults, file = table_file3)
...
file_path <- here("results", "figures","ROC3.png")
png(filename = file_path, width = 800, height = 600)
...
set.seed(24)
traindown1 = downSample(x=train_x2, 
                       y=train_y2)

table(train_y2)

table(traindown1$Class)
...
# MARS model
set.seed(21)
marsmodel2 = train(Class~., data=traindown1, 
                  method = "earth",
                  tuneGrid = expand.grid(degree=1, nprune=2:15),
                  trControl = ctrl)
...
file_path <- here("results", "figures","DSimportantM2.png")
png(filename = file_path, width = 800, height = 600)
plot(marsImp2)
dev.off()
...
knnModel2 = train(Class~., data=traindown1, 
                 method = "knn",
                 tuneGrid = data.frame(k=1:20),
                 trControl= ctrl)
...
file_path <- here("results", "figures","DSimportantKNN2.png")
png(filename = file_path, width = 800, height = 600)
plot(KNNImp2)
dev.off()
...
logregmodel2 = glm(Class~., data=traindown1, family = "binomial")
...
ggsave(here("results", "figures","DSimportantLog2.png"))



# Random Forest

rfmodel2 = randomForest(Class~., data = traindown1,
                       importance = TRUE,
                       preProcess = "pca",
                       trControl = ctrl)
...
ggsave(here("results", "figures","DSimportantRF2.png"))
...
table_file4 = here("results", "tables", "finaltable4.rds")
saveRDS(finalresults1, file = table_file4)
